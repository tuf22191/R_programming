...
for(x in 1:length(noRetweet1[,1])){
  if(counter==20){}
  else if (is.na(noRetweet1[x,"has_media"])){ 
    counter=counter+1
    ...
    Sys.sleep(2)
     ...
     vect <-return_media_info(json_result)
     noRetweet1[x,"has_media"]<-vect[[1]]
     noRetweet1[x,"media_type"]<-vect[[2]]
